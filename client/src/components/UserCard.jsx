...
const UserCard = ({ user }) => {
  ...
  return (
    <div key={user._id} className='shadow flex flex-col justify-between w-72 border border-gray-200 rounded-md p-4 pt-6'>
    <div className='text-center'>
        <img src={user.profile_picture} alt="" className='w-16 rounded-full shadow-md mx-auto'/>
        <p className='mt-4 font-semibold'>{user.full_name}</p>
        {user.username && <p className='font-light text-gray-500'>@{user.username}</p>}
        {user.bio && <p className='text-sm text-center px-4 text-gray-600 mt-2'>{user.bio}</p>}
    </div>
    ...
    <div className='flex mt-4 gap-2'>
        ...
        <button onClick={handleConnectionRequest} className='flex items-center justify-center w-16 border text-slate-500 group rounded-md cursor-pointer active-slate-95 transition '>
            {
