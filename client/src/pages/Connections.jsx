...
const Connections = () => {
    ...
    const dataArray = [
        ...
        // {lable: 'Messages',value: [], icon: MessageSquare},
    ]
    return ( 
        ...
        <div className='max-w-6xl mx-auto p-6'>
          {/* Title */}
           <div className='mb-8'>
                ...
                <p className='text-slate-600'>Manage your connections and discover new ones</p>
             </div>
            {/* Counts */}
            <div className='mb-8 flex flex-wrap gap-6'>
            {dataArray.map((item,index)=>(
                <div key={index} className='flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gray bg-white rounded-md shadow'>
            ...
            {dataArray.map((tab)=>(
                    <button onClick={()=>setCurrentTab(tab.lable)}
                    key={tab.lable} className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab===tab.lable ? 'bg-white text-black font-medium' : 'text-gray-500 hover:text-black'}`}>
                        <tab.icon className='w-4 h-4'/>
                        <span className='ml-1'>{tab.lable}</span>
                        {tab.count !== undefined && (
                            <span className='ml-2 text-xs bg-gray-100 text-gray-700 rounded-full px-2 py-0.5'>{tab.count}</span>
                        )}
                    </button>
                ))}
            </div>
            ...
            {dataArray.find(item=>item.lable===currentTab).value.map((user)=>(
                <div key={user._id} className='max-w-xs flex flex-col gap-4 p-6 bg-white rounded-md shadow'>
                  <img src={user.profile_picture} alt="" className='w-12 h-12 shadow-md mx-auto rounded-full' />
